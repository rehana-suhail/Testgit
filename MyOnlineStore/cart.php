...
if (isset($_POST['item_to_adjust']) && $_POST['item_to_adjust'] != "") {
	...
	foreach ($_SESSION["cart_array"] as $each_item) { 
		      $i++;
		      while (list($key, $value) = myEach($each_item)) {
...
// Created My own Function to replace each function
function myEach(&$arr) {
    $key = key($arr);
...
if (!isset($_SESSION["cart_array"]) || count($_SESSION["cart_array"]) < 1) {
    ...
    foreach ($_SESSION["cart_array"] as $each_item) { 
		...
		$cartTotal = (float)$pricetotal + (float)$cartTotal;
		setlocale(LC_MONETARY, "en_US");
        //$pricetotal = money_format("%10.2n", $pricetotal);
		$fmt = new NumberFormatter( 'en_US', NumberFormatter::CURRENCY );
		$pricetotal = $fmt->formatCurrency($pricetotal, 'USD');
	...
	$fmt = new NumberFormatter( 'en_US', NumberFormatter::CURRENCY );
	$cartTotal = $fmt->formatCurrency($cartTotal, 'USD');
